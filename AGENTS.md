...
- Separate staging also enables cross-process read coalescing. A bounded set
  of cache-local file locks makes one process fetch and publish each missing
  immutable block before peer mounts recheck the shared cache. Private cache
  layouts retain the ordinary in-process singleflight and asynchronous writer.
